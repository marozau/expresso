...
object ToKeyRecord {
  ...
  def toKeyImpl[T: c.WeakTypeTag](c: whitebox.Context): c.Expr[ToKeyRecord[T]] = {
    ...
    def isKeyRecordAnnotation(sym: Symbol): Boolean = sym.annotations.exists { a =>
      a.tree.tpe == typeOf[today.expresso.sqrs.api.Key]
    }
    ...
    c.Expr[ToKeyRecord[T]](
      q"""
         new _root_.today.expresso.sqrs.api.ToKeyRecord[$tType] {
             override def apply(t: $tType) = com.sksamuel.avro4s.ToRecord[_root_.today.expresso.sqrs.api.KeyRecord[$fType]](_root_.today.expresso.sqrs.api.KeyRecord[$fType](t.$termName))
         }
        """
    )
  }
