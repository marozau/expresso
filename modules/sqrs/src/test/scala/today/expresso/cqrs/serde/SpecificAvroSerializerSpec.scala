package today.expresso.cqrs.serde
...
import today.expresso.cqrs.api._
import today.expresso.cqrs.serde.specific.{GenericAvroSerializer, SpecificAvroDeserializer, SpecificAvroSerializer}
import today.expresso.cqrs.serde.utils.{GenericAvroUtils, SpecificAvroUtils}
...
class SpecificAvroSerializerSpec extends WordSpec
  ...
  "SpecificAvroSerializer" must {
    ...
    "serialize using generic record as intermediate citizen" in {
      ...
      record.getSchema.getFullName should be("today.expresso.cqrs.serde.TestObject")
