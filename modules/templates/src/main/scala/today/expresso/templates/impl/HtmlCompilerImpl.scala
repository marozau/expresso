...
import today.expresso.templates._
...
class HtmlCompilerImpl(directory: String, injector: Injector)(implicit ec: ExecutionContext) extends HtmlCompiler {
  ...
  override def compileAsync(html: String, packageName: Option[String] = None, imports: Seq[String] = List.empty): Future[HtmlTemplate] = {
    Future {
      compile(html, packageName, imports)
    }(ec)
  }
}
