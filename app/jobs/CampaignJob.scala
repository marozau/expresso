...
object CampaignJob {
  ...
  def toInstant(campaign: Campaign) = {
    ...
    val now = Instant.now()
    if (campaignTimestamp.compareTo(now) < 0) throw InvalidCampaignScheduleException(campaign.id.get, campaign.sendTime, "campaign timestamp must be in the future")
